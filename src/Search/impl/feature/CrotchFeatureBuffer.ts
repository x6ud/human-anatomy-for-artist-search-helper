...
export default class CrotchFeatureBuffer implements FeatureBuffer {

    filename: string = 'crotch';
    ...
    create(chunk: PhotoPoseLandmarks[]): Float32Array {
        ...
        for (let i = 0, len = chunk.length; i < len; ++i) {
            ...
            const crotchUp = getNormal(mid(leftHip, rightHip), mid(leftShoulder, rightShoulder));
            const crotchRight = getNormal(rightHip, leftHip);
            const rotation = getQuatFromRightUp(crotchRight, crotchUp);
    ...
    getCrotchRotation(buffer: Float32Array, i: number): [number, number, number, number] {
        const offset = i * BUFFER_STEP;
        return [
            buffer[offset + 0],
            buffer[offset + 1],
